...
- [Log-Structured Storage Engines](#log-structured-storage-engines)
  - [Indexing](#indexing)
    ...
    - [SSTables and LSM-Trees](#sstables-and-lsm-trees)
      ...
      - [Performance Optimizations (Bloom filters)](#performance-optimizations-bloom-filters)
      - [Performance Optimizations (Compaction)](#performance-optimizations-compaction)
    - [B-Trees](#b-trees)
    ...
    - [Other Indexing Structures](#other-indexing-structures)
      ...
      - [Keep everything in memory (In memory database)](#keep-everything-in-memory-in-memory-database)
...
#### memory consumption   
5. You don't need to keep an index of all the keys in memory. Because the file is sorted, if you're looking for the offset of a particular key, it won't be difficult to find that offset once you can determine the offset of keys that are smaller and larger than it in the ordering.
6. You still need an in-memory index to tell you the offsets of some keys, but it can be sparse.
...
7. SSTables store their keys in blocks, and have an internal index, so even though a single SSTable may be very large (gigabytes in size), only the index and the relevant block needs to be loaded into memory.
