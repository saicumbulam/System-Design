...
class DeferredCallbackExecutor {
    ...
    public void start() throws InterruptedException {
        ...
        while (true) {
            ...
            while (q.size() != 0) {
                ...
                newCallbackArrived.await(sleepFor, TimeUnit.MILLISECONDS); // put the thread to sleep
            }

            CallBack cb = q.poll(); // queue the callback from the queue
            System.out.println(
                    "Executed at " +
    ...
    public void registerCallback(CallBack callBack) {
        lock.lock();
        q.add(callBack);
        newCallbackArrived.signal(); // signal the condition that a new item has arroved
        lock.unlock();
    }
